...
const NotificationDropdown = () => {
  return <div id="notification-dropdown-people" className="topbar-item">
    <Dropdown>
      ...
      <DropdownMenu className="p-0 dropdown-menu-end dropdown-menu-lg">
        ...
        <SimpleBar style={{
          ...
          {notifications.map(notif => <DropdownItem key={notif.id} id={notif.id} className="notification-item py-2 text-wrap">
            <span className="d-flex align-items-center gap-3">
              <span className="flex-shrink-0 position-relative">
                {notif.avatarType === "image" && notif.avatarSrc ? <img src={notif.avatarSrc} alt={`${notif.name} Avatar`} className="avatar-md rounded-circle" width={48} height={48} /> : <span className={`avatar-md rounded-circle ${notif.avatarBg} d-flex align-items-center justify-content-center`}>
                  <Icon icon={notif.avatarIcon ?? ""} className="fs-4" />
                </span>}
              ...
              <span className="flex-grow-1 text-muted">
                ...
                {notif.message}{" "}
                {notif.project && <span className="fw-medium text-body">
                  {notif.project}
                </span>}
                <br />
