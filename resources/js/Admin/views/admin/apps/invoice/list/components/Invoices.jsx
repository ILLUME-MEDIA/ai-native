  ...
  const columns = [{
    ...
    cell: ({
      row
          ...
          {row.original.user.image ? <div className="avatar avatar-sm">
    ...
    cell: ({
      row
    }) => <span className={`badge ${row.original.status === 'paid' ? 'bg-success-subtle text-success' : row.original.status === 'pending' ? 'bg-warning-subtle text-warning' : row.original.status === 'overdue' ? 'bg-danger-subtle text-danger' : row.original.status === 'draft' ? 'bg-info-subtle text-info' : 'bg-secondary-subtle text-secondary'} badge-label`}>
