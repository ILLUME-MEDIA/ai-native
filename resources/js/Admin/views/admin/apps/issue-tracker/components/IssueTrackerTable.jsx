  ...
  const columns = [columnHelper.accessor('status', {
    ...
    cell: ({
      row
          ...
          <Link to="#" onClick={toggleDetailModal} className="link-reset text-uppercase fw-semibold">
            {row.original.id}
          </Link>
    ...
    cell: ({
      row
          ...
          <ProgressBar now={row.original.progress} variant={row.original.variant} style={{
        height: '6px'
      }} />
    ...
    cell: ({
      row
          ...
          <button className="btn btn-default btn-icon btn-sm" onClick={() => {
        toggleDeleteModal();
        setSelectedRowIds({
          [row.id]: true
        });
