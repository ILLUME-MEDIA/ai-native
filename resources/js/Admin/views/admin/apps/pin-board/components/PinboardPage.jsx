          ...
          <Card className="bg-transparent shadow-none">
            <CardBody>
              <div className="pin-board d-flex flex-wrap">
                {pinNotes.map(note => <div key={note.id} className={clsx('pin-board-item', note.className)}>
                    <p className="text-end fs-10">{note.timestamp}</p>
                    <h4 className="fs-md">{note.title}</h4>
                    <p>{note.description}</p>
                    <Link to="" className="pin-board-delete fs-lg" onClick={e => {
                  e.stopPropagation();
...
const AddPinNoteModal = ({
  show,
  toggleModal,
  handleSubmit
}) => {
  return <Modal show={show} onHide={toggleModal} centered aria-labelledby="addNoteModalLabel">
      <ModalHeader>
