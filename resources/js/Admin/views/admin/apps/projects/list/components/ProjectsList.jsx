...
const ProjectsList = () => {
  const columns = [{
    ...
    cell: ({
      row
    }) => <span className={`badge badge-soft-${getVariant(row.original.status)} fs-xxs badge-label`}>{toPascalCase(row.original.status)}</span>
    ...
    cell: ({
      row
    }) => <ProgressBar variant={getVariant(row.original.status)} now={row.original.progress} style={{
      height: '5px'
    }} />,
    enableSorting: false
  ...
  return <Card>
      <CardHeader className="border-light justify-content-between">
        ...
        <div className="d-flex align-items-center gap-2">
          ...
          <div className="app-search">
            <FormSelect value={table.getColumn('dueDate')?.getFilterValue() ?? ''} onChange={e => table.getColumn('dueDate')?.setFilterValue(e.target.value || undefined)} className="form-control my-1 my-md-0">
