...
import SectionBuilder, { normalizeSection } from './SectionBuilder';
...
const CaseStudyEdit = () => {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const res = await axios.get(`/api/admin/case-studies/${id}`);
        const item = res.data;
        ...
        // Normalize tags: accept [string] or [{value}] format
        tags = tags.map((t) => (typeof t === 'string' ? t : t.value ?? t.label ?? String(t)));
        ...
        setSections(rawSections.map((s) => normalizeSection({
          id:        s.id ?? null,
          type:      s.type ?? 'text',
          heading:   s.heading ?? '',
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      await axios.post(`/api/admin/case-studies/${id}/update`, fd, {
  ...
  if (loading) {
    return (
      <div className="d-flex justify-content-center align-items-center py-5">
        ...
        <span className="ms-3 text-muted">Loading case study...</span>
      </div>
    );
  }
  ...
  return (
    <>
      <PageBreadcrumb title={form.title || 'Edit Case Study'} subtitle="Case Studies" />
                ...
                <div className="d-grid gap-2">
                  <Button type="submit" variant="primary" disabled={saving}>
                    {saving
                      ...
                      : <><Icon icon="device-floppy" className="me-2" />Update Case Study</>}
                  </Button>
