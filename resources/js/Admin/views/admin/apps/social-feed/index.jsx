  ...
  return <Card>
      <CardBody>
        ...
        <ul className="list-unstyled mt-2 mb-0">
          {activityData.map((item, idx) => <li className="d-flex align-items-center py-1" key={idx}>
...
const Trending = () => {
  return <Card>
      ...
      <CardBody>
        {trendingData.map((item, idx) => <div className={clsx('d-flex', {
...
const Requests = () => {
  return <Card>
      ...
      <CardBody>
        {requestData.map((item, idx) => <div key={idx} className={`d-flex justify-content-between align-items-center ${idx !== requestData.length - 1 ? 'mb-3' : ''}`}>
            <div className={`d-flex align-items-start ${item.icon ? 'gap-2' : ''}`}>
              {item.image ? <img src={item.image} alt={item.title} width={32} height={32} className="avatar-xs rounded-circle me-2" /> : <div className="avatar-xs flex-shrink-0">
