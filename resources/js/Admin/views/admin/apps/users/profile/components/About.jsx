...
const About = () => {
  return <>
      ...
      <div className="timeline">
        {timelineData.map((item, index) => <div key={index} className="timeline-item d-flex align-items-stretch">
      ...
      <Table responsive className="table-centered table-custom table-sm table-nowrap table-hover mb-0">
        ...
        <tbody>
          {taskData.map((task, idx) => <tr key={idx}>
              <td>
              ...
              <td>
                <span className={`badge ${task.status === 'outdated' ? 'badge-soft-danger' : task.status === 'in-progress' ? 'badge-soft-warning' : task.status === 'on-hold' ? 'badge-soft-dark' : 'badge-soft-success'}`}>{task.status}</span>
              </td>
              <td>
              ...
              <td>
                <span className={`badge ${task.priority === 'high' ? 'badge-soft-danger' : task.priority === 'medium' ? 'badge-soft-secondary' : 'badge-soft-success'}`}>{task.priority}</span>
              </td>
              <td>{task.progress}</td>
              <td>{task.time}</td>
              <td>
                <Link to="" className="text-muted fs-xxl">
                  <Icon icon="edit" className="align-middle" />
                </Link>
              </td>
            </tr>)}
        </tbody>
