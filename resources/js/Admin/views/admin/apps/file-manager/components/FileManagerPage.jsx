...
import { formatBytes } from '@admin/utils/helpers';
      ...
      <Card className="h-100 mb-0 border-start-0 flex-grow-1 rounded-start-0">
        ...
        height: 'calc(100% - 100px)'
          ...
          <Row className="g-2 mb-3">
            {folderData.map((folder, idx) => <Col md={6} lg={4} xxl={3} key={idx}>
  ...
  return <SimpleBar className="card h-100 mb-0 rounded-end-0">
      <CardBody>
        ...
        <ListGroup variant="flush" className="list-custom mt-3">
          {sidebarMenuItemData.map((item, idx) => <ListGroupItem key={item.name} as={Link} href="" action className={idx === 0 ? 'active' : ''}>
          ...
          {categoryData.map(item => <ListGroupItem key={item.name} as={Link} href="" action>
