...
const ExpensesTable = () => {
  ...
  const columns = [{
    ...
    cell: ({
      row
    }) => <span className={`badge badge-label ${row.original.status === 'approved' ? 'bg-success-subtle text-success' : row.original.status === 'pending' ? 'bg-warning-subtle text-warning' : row.original.status === 'rejected' ? 'bg-danger-subtle text-danger' : row.original.status === 'reimbursed' ? 'bg-success-subtle text-success' : ''}`}>
    ...
    cell: ({
      row
          ...
          <img src={row.original.addedBy.image} alt={row.original.addedBy.name} className="avatar-xs rounded-circle" />
