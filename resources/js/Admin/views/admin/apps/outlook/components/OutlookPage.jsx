...
const OutlookPage = () => {
  const [message, setMessages] = useState(messageData[0]);
  ...
  return <SimpleBar className="card h-100 mb-0 rounded-0 border-0">
      <CardBody className="p-0">
        <ListGroup as="ul" variant="flush" className="outlook-list mb-0">
          {messages.map((item, idx) => <ListGroupItem as="li" key={idx} className="p-0" role="button" onClick={() => {
          setMessages(item);
          setActive(item);
        }}>
              <span className={clsx('nav-link p-3', {
