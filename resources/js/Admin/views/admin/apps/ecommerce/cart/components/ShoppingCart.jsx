...
const ShoppingCart = () => {
  ...
  const handleQuantityChange = (id, newQuantity) => {
    setCartItem(prev => prev.map(item => item.id === id ? {
      ...item,
  ...
  return <Card>
      <CardHeader className="d-flex justify-content-between align-items-center">
        ...
        <a href="#" className="text-decoration-underline link-offset-2 fw-medium">
          Clear cart
        </a>
      </CardHeader>
              ...
              {cartItem.map(item => <tr key={item.id}>
                  ...
                  <td>
                    {item.discount ? <>
                        ...
                        <br />
                        <small className="text-decoration-line-through text-muted">${(item.price - item.price * (item.discount / 100)).toFixed(2)}</small>
