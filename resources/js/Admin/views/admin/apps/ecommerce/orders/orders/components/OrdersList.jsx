...
const OrdersList = () => {
  const columns = [{
    ...
    cell: ({
      row
    }) => <span className={`badge fs-xxs badge-soft-${row.original.orderStatus === 'cancelled' ? 'danger' : row.original.orderStatus === 'processing' ? 'warning' : 'success'}`}>{toPascalCase(row.original.orderStatus)}</span>
  ...
  return <Card>
      <CardHeader className="border-light justify-content-between">
        ...
        <div className="d-flex gap-1">
          <Link to="/apps/ecommerce/product-add" className="ms-1 btn btn-danger">
