...
const ProductStockTable = () => {
  const columns = [{
    ...
    cell: ({
      row
    }) => <h5 className={clsx('fs-base mb-0 fw-medium', row.original.availableStock === 0 ? 'text-danger' : 'text-success')}>{row.original.availableStock}</h5>
  }), columnHelper.accessor('lowStock', {
    header: 'Low Stock',
    cell: ({
      row
    }) => <h5 className={clsx('fs-base mb-0 fw-medium', row.original.lowStock < 20 ? 'text-danger' : 'text-warning')}>{row.original.lowStock}</h5>
    ...
    cell: ({
      row
    }) => <span className={`badge ${row.original.status === 'out-of-stock' ? 'badge-soft-danger' : row.original.status === 'low-stock' ? 'badge-soft-warning' : 'badge-soft-success'} fs-xxs`}>{toPascalCase(row.original.status)}</span>
  ...
  return <Card>
      <CardHeader className="border-light justify-content-between">
        ...
        <div className="d-flex align-items-center gap-2 flex-wrap">
          ...
          <div className="app-search">
            <FormSelect value={table.getColumn('category')?.getFilterValue() ?? 'All'} onChange={e => table.getColumn('category')?.setFilterValue(e.target.value === 'All' ? undefined : e.target.value)} className="form-control my-1 my-md-0">
