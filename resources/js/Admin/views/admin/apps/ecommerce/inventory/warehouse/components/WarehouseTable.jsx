...
const WarehouseTable = () => {
  const columns = [{
    ...
    cell: ({
      row
    }) => <h5 className="fs-sm mb-0">{row.original.id}</h5>
    ...
    cell: ({
      row
    }) => <>{row.original.shippingStock} units</>
    ...
    cell: ({
      row
    }) => <span className={`badge ${row.original.status === 'closed' ? 'badge-soft-danger' : row.original.status === 'maintenance' ? 'badge-soft-warning' : 'badge-soft-success'} fs-xxs`}>{toPascalCase(row.original.status)}</span>
  }), {
    header: 'Actions',
    cell: ({
      row
  ...
  return <Card>
      <CardHeader className="border-light justify-content-between">
        ...
        <div className="d-flex align-items-center gap-2 flex-wrap">
          ...
          <div className="app-search">
            <FormSelect value={table.getColumn('location')?.getFilterValue() ?? 'All'} onChange={e => table.getColumn('location')?.setFilterValue(e.target.value === 'All' ? undefined : e.target.value)} className="form-control my-1 my-md-0">
