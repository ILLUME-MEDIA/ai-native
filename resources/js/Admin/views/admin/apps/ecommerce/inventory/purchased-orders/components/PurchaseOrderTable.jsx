...
const PurchaseOrderTable = () => {
  const columns = [{
    ...
    cell: ({
      row
    }) => <h5 className="fs-base mb-0 fw-medium">{row.original.amount}</h5>
    ...
    cell: ({
      row
    }) => <span className={`badge badge-label ${row.original.paymentStatus === 'pending' ? 'badge-soft-warning' : row.original.paymentStatus === 'paid' ? 'badge-soft-success' : 'badge-soft-danger'} fs-xxs`}>{toPascalCase(row.original.paymentStatus)}</span>
    ...
    cell: ({
      row
    }) => <span className={`badge badge-label ${row.original.orderStatus === 'cancelled' ? 'badge-soft-danger' : row.original.orderStatus === 'in-progress' ? 'badge-soft-warning' : row.original.orderStatus === 'partially-received' ? 'badge-soft-info' : 'badge-soft-success'} fs-xxs`}>
  ...
  return <Card>
      <CardHeader className="border-light justify-content-between">
        ...
        <div className="d-flex align-items-center gap-2 flex-wrap">
          ...
          <div className="app-search">
            <FormSelect value={table.getColumn('paymentStatus')?.getFilterValue() ?? 'All'} onChange={e => table.getColumn('paymentStatus')?.setFilterValue(e.target.value === 'All' ? undefined : e.target.value)} className="form-control my-1 my-md-0">
          ...
          <div className="app-search">
            <FormSelect value={table.getColumn('orderStatus')?.getFilterValue() ?? 'All'} onChange={e => table.getColumn('orderStatus')?.setFilterValue(e.target.value === 'All' ? undefined : e.target.value)} className="form-control my-1 my-md-0">
