...
const RefundTable = () => {
  const columns = [{
    ...
    cell: ({
      row
    }) => <span className={`badge ${row.original.status === 'rejected' ? 'badge-soft-danger' : row.original.status === 'pending' ? 'badge-soft-warning' : row.original.status === 'refunded' ? 'badge-soft-secondary' : 'badge-soft-success'} fs-xxs`}>
    ...
    cell: ({
      row
    }) => row.original.processedDate ? row.original.processedDate : '-'
  }), {
    header: 'Actions',
    cell: ({
      row
  ...
  return <Card>
      ...
      {table.getRowModel().rows.length > 0 && <CardFooter className="border-0">
          <TablePagination totalItems={totalItems} start={start} end={end} itemsName="refunds" showInfo previousPage={table.previousPage} canPreviousPage={table.getCanPreviousPage()} pageCount={table.getPageCount()} pageIndex={table.getState().pagination.pageIndex} setPageIndex={table.setPageIndex} nextPage={table.nextPage} canNextPage={table.getCanNextPage()} />
        </CardFooter>}
