...
const AttributeTable = () => {
  const columns = [{
    ...
    cell: ({
      row
    }) => <h6 className="mb-0 fw-medium fs-base">{row.original.attribute}</h6>
    ...
    cell: ({
      row
    }) => <span className="text-muted">{row.original.options.join(', ')}</span>
  }), columnHelper.accessor('isActive', {
    header: 'Status',
    cell: ({
      row
    }) => <FormCheck type="switch" defaultChecked={row.original.isActive} />
